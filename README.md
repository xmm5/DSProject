...
Для настройки и запуска проекта нужно выполнить в консоли следующие команды:

```
mkdir -p /path/to/project/dir
cd /path/to/project/dir
git clone https://github.com/xmm5/DSProject.git /path/to/project/dir
python -m venv ./venv
...
После выполнения данных команд на локальном компьютере будет запущен локальный
web-сервер.

Для вызова REST-сервиса, для поиска близких вакансий, можно выполнить в консоли,
следующий запрос:

```
curl --location --request POST 'http://127.0.0.1:8080/api/getSimilarVacancies' \
